...
void Pontus::Query(val_tp thresh, std::vector<std::pair<key_tp, val_tp>> &results)
{
	myset res;
	for (int i = 0; i < pontus_.width * pontus_.depth; i++)
	{

		if (pontus_.counts[i]->count > (int)thresh)
		{
			key_tp reskey;
...
/*void Pontus::MergeAll(Pontus** pontus_arr, int size) {
  long countarr[size];
  val_tp sumarr[size];
  unsigned char* keyarr[size];
  long est[size];

  for (int d = 0; d < pontus_.depth; d++) {
	for (int w = 0; w < pontus_.width; w++) {
	  val_tp total = 0;
	  for (int i = 0; i < size; i++) {
		//find the majority and its estimate
		Pontus* cursk = pontus_arr[i];
		Pontus::SBucket** table = (cursk)->GetTable();
		int index = d*pontus_.width+w;
	  ...
	  for (int i = 0; i  < size; i++) {
		...
		for (int j = 0; j < size; j++) {
		  if (memcmp(keyarr[i], keyarr[j], pontus_.lgn/8) == 0) {
